...
-- nomor 2
SELECT CONCAT(e.firstName, ' ', e.lastName) AS 'nama pegawai', SUM(pm.amount) 'gaji',
	CASE
		WHEN SUM(pm.amount) > (SELECT AVG(total_gaji) FROM
		                                              (SELECT SUM(pm.amount) AS total_gaji 
																	 FROM payments pm
																	 JOIN customers c 
...
SELECT c.customerName AS 'Nama Pelanggan',
   ...
   CASE
      WHEN MONTH(o.orderDate) % 2 = 1 AND
         SUM(DATEDIFF(o.shippedDate, o.orderDate)) > (SELECT AVG(total) FROM (SELECT SUM(DATEDIFF(shippedDate, orderDate)) AS 'total' 
			FROM orders 
...
SELECT c.customerName,
   ...
   CASE
      WHEN MONTH(o.orderDate) % 2 = 1 AND
      	COUNT(p.productCode) > (SELECT AVG(total) * 10 FROM (SELECT COUNT(productName) AS total 
			FROM products 
